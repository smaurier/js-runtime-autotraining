...
function slowBuildReport(items: any[]): string {
  let report = '';
...
function slowTransformData(items: any[]): any[] {
  const results = [];
  for (let i = 0; i < items.length; i++) {
...
function memoFibonacci(n: number, cache: Map<number, number> = new Map()): number {
  if (n <= 1) return n;
...
function fastBuildReport(items: any[]): string {
  const parts = [];
  for (let i = 0; i < items.length; i++) {
    parts.push(
...
function fastTransformData(items: any[]): any[] {
  const results = [];
