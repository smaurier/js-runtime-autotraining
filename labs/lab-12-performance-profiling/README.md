...
- Maitriser le profiler V8 integre (`--prof` et `--prof-process`)
- Savoir lire un rapport de tick processor et identifier les goulots d'etranglement
- Corriger 4 problemes de performance dans un programme deliberement lent
- Utiliser `performance.mark()` / `performance.measure()` pour instrumenter le code
- Rediger un rapport de synthese comparant avant/apres
...
| 3 | Instrumenter avec performance.mark/measure, comparer avant/apres |
| 4 | Rediger un rapport de synthese des optimisations |

## Les 4 goulots d'etranglement

1. **Fibonacci recursif** sans memoisation — complexite exponentielle O(2^n)
2. **Concatenation de chaines** avec `+=` en boucle — allocation O(n^2)
3. **JSON.parse/stringify** dans un chemin chaud — parsing inutile a chaque iteration
...
- **total** : pourcentage du temps total d'execution
...
- Un avant/apres avec performance.mark/measure est presente
