...
class MiniEventLoop {
  constructor() {
    ...
    // Chaque element est un nom de fonction/tache pour le logging.
    this.callStack = [];

    // File FIFO des macrotaches (setTimeout, setInterval, I/O callbacks).
    // Chaque element : { fn: Function, delay: number, name: string, order: number }
    // `order` sert a maintenir l'ordre d'insertion pour les memes delays.
    this.macroTaskQueue = [];

    // File FIFO des microtaches (Promise.then, queueMicrotask).
    // Chaque element : { fn: Function, name: string }
    this.microTaskQueue = [];

    // File FIFO des process.nextTick — prioritaires sur les autres microtaches.
    // Dans Node.js, nextTick est execute AVANT les Promise.then a chaque cycle.
    // Chaque element : { fn: Function, name: string }
    this.nextTickQueue = [];
    ...
    // C'est ce tableau qui est retourne par run() et compare aux resultats attendus.
    this.outputLog = [];
  ...
  _createContext() {
    ...
    return {
       ...
       */
      setTimeout(fn, delay = 0, name = "setTimeout") {
       ...
       * @param {string} [name] - Nom pour le debug
       */
      promiseThen(fn, name = "promise.then") {
       ...
       */
      queueMicrotask(fn, name = "microtask") {
        self.microTaskQueue.push({ fn, name });
        self.executionLog.push(`[MICRO] Planifie: ${name}`);
      },
    };
