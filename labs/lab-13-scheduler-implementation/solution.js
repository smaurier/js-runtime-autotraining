...
class Scheduler {
   ...
   * @param {number} options.concurrency - Nombre max de taches simultanees (defaut: 3)
   */
   ...
   */
  postTask(fn, { priority = "normal", signal } = {}) {
    // Valider la priorite
    ...
    const task = { fn, priority, signal, resolve, reject };
...
async function assertRejects(promise, errorCheck, message) {
  try {
    await promise;
