...
//
// Commandes :
//   node solution.js                          (executer la comparaison)
...
function handleRequestOriginal(requestId, payload) {
...
function processRecordOriginal(record) {
...
function generateRecordsOriginal(count) {
  const records = [];
...
function broadcastMessagesOriginal(clientCount, iterations) {
  let totalLength = 0;
...
function readValueOriginal(obj) {
  return obj.value;
}

function processHeterogeneousObjectsOriginal(count) {
  const objects = [];
  for (let i = 0; i < count; i++) {
    switch (i % 8) {
...
function processLargePayloadOriginal() {
...
function handleConnectionOriginal(connectionId) {
...
function simulateConnectionsOriginal(count) {
  for (let i = 0; i < count; i++) {
...
function processRecordFixed(record) {
...
function simulateConnectionsFixed(count) {
  for (let i = 0; i < count; i++) {
