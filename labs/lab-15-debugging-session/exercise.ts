...
//
// Commandes utiles :
//   npx tsx exercise.ts                          (observer les symptomes)
...
function handleRequest(requestId: number, payload: any): { status: string; id: number } {
...
function broadcastMessages(clientCount: number, iterations: number): number {
  let totalLength = 0;
...
function readValue(obj: { value: number }): number {
  return obj.value;
}

function processHeterogeneousObjects(count: number): number {
...
function processLargePayload(): { totalProcessed: number; originalSize: number } {
...
function handleConnection(connectionId: number): void {
...
function simulateConnections(count: number): number {
  for (let i = 0; i < count; i++) {
    handleConnection(i);
  }
...
function runDiagnosticPipeline(): void {
  console.log('=================================================================');
