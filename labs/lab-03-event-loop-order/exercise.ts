...
function checkPrediction(snippetName: string, prediction: any[], actual: any[], phaseMap: Record<string, string>): void {
  total++;
...
function runSnippet(name: string, prediction: any[], fn: (log: (msg: any) => void) => void, phaseMap?: Record<string, string>): Promise<void> {
...
function provePhaseOrder(): Promise<void> {
  return new Promise((resolve) => {
    const order = [];
...
function demonstrateStarvation(): Promise<void> {
  return new Promise((resolve) => {
